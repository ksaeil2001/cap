...
import { Check, Info, Plus, AlertTriangle, DollarSign } from 'lucide-react';
...
const FoodCard: React.FC<FoodCardProps> = ({
  ...
  return (
    <Card className={cn(
      ...
      isSelected ? "ring-2 ring-primary shadow-md" : variant === 'default' && "hover:translate-y-[-2px]"
    )}>
      {/* Selection Indicator */}
      {isSelected && (
      ...
      <div className="relative h-48 overflow-hidden bg-gray-100">
        <img 
          ...
          className={cn(
            ...
            variant === 'default' && "transition-transform hover:scale-105"
          )}
          onError={(e) => {
      ...
      {showActions && (
        <CardFooter className="p-3 pt-0 flex justify-between gap-2">
          ...
          {onSelect && (
            <Button 
              variant={variant === 'display-only' ? "destructive" : isSelected ? "destructive" : "default"}
              size="sm"
              className="text-xs flex-1"
              onClick={() => onSelect(food)}
            >
              {variant === 'display-only' || isSelected ? (
                <>
